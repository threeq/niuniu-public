...
Personal 桌面版是牛牛的单机本地版本，以原生桌面应用交付（Tauri v2，内置本地服务）：所有数据存在你电脑的 SQLite 文件里，不需要服务器、不需要 PostgreSQL，装好就能用。

本文按"装好桌面版 → 备齐依赖 → 配好账号"三步走，跟着做完即可在 5 分钟内启动第一个 agent。
...
- **Windows** 10+ x64 —— 需要 [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)
  ...
  - 如果缺失，牛牛桌面版启动时会弹出**原生提示框**引导你打开下载页 —— 安装后重新启动即可
...
访问 [GitHub Releases](https://github.com/threeq/niuniu/releases)，找到最新版本，下载对应平台的安装包：

| 平台 | 文件名 |
|---|---|
| macOS Apple Silicon | `niuniu-desktop-v2-<版本>-darwin-arm64.dmg` |
| macOS Intel | `niuniu-desktop-v2-<版本>-darwin-amd64.dmg` |
| Windows | `niuniu-desktop-v2-<版本>-windows-amd64.exe` |
| Linux | `niuniu-desktop-v2-<版本>-linux-amd64.AppImage` |

> `<版本>` 即 release tag，例如 `v0.8.7`。

### 国内镜像（百度网盘）

访问 GitHub 较慢或受限时，可从百度网盘下载同一版本的安装包：
...
**macOS**：打开 `.dmg`，把牛牛拖入「应用程序」后启动。
**Windows**：双击 `.exe` 安装，可能需要批准 SmartScreen 提示。
**Linux**：给 `.AppImage` 加可执行权限（`chmod +x niuniu-desktop-v2-*.AppImage`）后直接运行。macOS 首次运行可能需要在 系统设置 → 隐私与安全性 里允许"开发者未签名"。
**Windows**：双击 `.exe`，可能需要批准 SmartScreen 提示。

启动后进入牛牛桌面窗口（应用内置本地服务，数据仍全部存放在本机）。个人版默认无需登录账号，直接进入主界面即可。
...
为了统计活跃用户数与版本 / 系统分布，个人版在启动时会发送**一条匿名的「打开事件」**（仅 6 个字段，不含账号、用量、仓库内容等任何隐私信息）。这是 **opt-out** 设计：在 **设置 → 隐私** 关闭「匿名使用统计」开关即可停止上报，下一个周期立即生效。完整说明（采集什么、绝不采集什么、保留期、如何请求删除）见 [遥测与隐私说明](/docs/telemetry-privacy)。
