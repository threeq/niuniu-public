...
Personal is Niuniu's single-machine local edition, shipped as a native desktop app (Tauri v2 with a bundled local server): all data lives in a SQLite file on your computer — no server, no PostgreSQL required. Install, launch, and you're ready to go.

This guide walks you through three stages: **install the desktop app → set up dependencies → configure your account**. Follow it end-to-end and you'll have an agent running in under five minutes.
...
- **Windows** 10+ x64 — requires the [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)
  ...
  - If missing, the desktop app will show a **native dialog** with a button to open the download page — install it, then relaunch
...
Go to [GitHub Releases](https://github.com/threeq/niuniu/releases), find the latest release, and download the installer for your platform:

| Platform | File |
|---|---|
| macOS Apple Silicon | `niuniu-desktop-v2-<version>-darwin-arm64.dmg` |
| macOS Intel | `niuniu-desktop-v2-<version>-darwin-amd64.dmg` |
| Windows | `niuniu-desktop-v2-<version>-windows-amd64.exe` |
| Linux | `niuniu-desktop-v2-<version>-linux-amd64.AppImage` |

> `<version>` is the release tag, e.g. `v0.8.7`.

### Mainland China mirror (Baidu Netdisk)

If GitHub is slow or unreachable, the same installers are mirrored on Baidu Netdisk:
...
**macOS**: open the `.dmg` and drag Niuniu into Applications, then launch. You may need to allow the unsigned app in System Settings → Privacy & Security on first run.
**Windows**: double-click the `.exe` to install — you may need to approve the SmartScreen prompt.
**Linux**: mark the `.AppImage` executable (`chmod +x niuniu-desktop-v2-*.AppImage`) and run it directly.

After launch, the Niuniu desktop window opens (a local server is bundled; your data stays on your machine). The Personal edition runs without any login — you'll land directly on the main console.
