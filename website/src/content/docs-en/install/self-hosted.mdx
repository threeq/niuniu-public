...
**Option 2: desktop installers**

[GitHub Releases](https://github.com/threeq/niuniu/releases) ships macOS / Windows / Linux desktop installers (with the server bundled — fine for single-machine use); bare server binaries are not published with releases, so Linux servers should use the source build above.
